...
function analysis(users) {
  const now = Math.max(
    ...users.flatMap((user) =>
      user.posts.map((post) => new Date(post.timestamp).getTime())
    )
  );

  console.log("Latest Timestamp:", new Date(now)); 
  const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000; // 7 days before latest post
  ...
  // 2. Extract popular posts (at least 10 likes)
  const popularPostsPerActiveUser = activeUsers.map((user) => ({
    id: user.id,
    name: user.name,
  ...
  // 3. Calculate average likes per active user
  const totalLikes = popularPostsPerActiveUser.reduce((acc, user) => {
    const userLikes = user.posts.reduce((sum, post) => sum + post.likes, 0);
    return acc + userLikes;
  }, 0);

  const averageLikes = popularPostsPerActiveUser.length
    ? totalLikes / popularPostsPerActiveUser.length
    : 0;

  return {
    activeUsersCount: activeUsers.length,
    popularPostsPerActiveUser,
    averageLikes,
  };
}

// Run analysis and store the results
